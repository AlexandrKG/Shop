...
public class Queries implements DBCoordinator{
    ...
    public Queries(ShopDB shopDB) {
        this.shopDB = shopDB;
        try {
            dataSourceConnection = new DataSourceConnection(shopDB.getFlagDB());
        } catch (IOException | SQLException | PropertyVetoException e) {
            e.printStackTrace();
        }
    }

    public void initData(boolean resetData) {
        if(!shopDB.getFlagDB().equals("MySQL")) {
            derbyDB = new ShopDerbyInit(dataSourceConnection);
            derbyDB.initData();
        } else if(resetData) {
            mySQLDB = new ShopSQLGenerator(dataSourceConnection);
            mySQLDB.initData();
        }
        loaderData = new LoaderData(shopDB);
